import warnings
import bpy

# Translation functions
_      = bpy.app.translations.pgettext
iface_ = bpy.app.translations.pgettext_iface
tip_   = bpy.app.translations.pgettext_tip
data_  = bpy.app.translations.pgettext_data


def __translate_safe(func, msgid, msgctxt=None):
    try:
        return func(msgid=msgid, msgctxt=msgctxt)
    except UnicodeDecodeError as ex:
        warnings.warn(
            f"Translation UnicodeDecodeError: msgid={msgid!r}, error={ex}",
            category=RuntimeWarning, stacklevel=2)
        return msgid


def __format_with_translation_function(func):
    # create a new format function using the specified function for translation
    def _format(msgid: str, *args, msgctxt=None, **kwargs) -> str:
        f_msg = __translate_safe(func, msgid, msgctxt=msgctxt)
        f_args = [
            __translate_safe(func, arg, msgctxt=msgctxt) if isinstance(arg, str) else arg
            for arg in args
        ]
        f_kwargs = {
            key: __translate_safe(func, arg, msgctxt=msgctxt) if isinstance(arg, str) else arg
            for key, arg in kwargs.items()
        }
        try:
            return f_msg.format(*f_args, **f_kwargs)
        except KeyError as ex:
            missing_key = ex.args[0] if ex.args else '<unknown>'
            warnings.warn(
                f"Translation format KeyError: missing='{missing_key}', kwargs={sorted(f_kwargs.keys())}",
                category=RuntimeWarning, stacklevel=2)
            return f_msg

    return _format


# Format translation functions
f_       = __format_with_translation_function( _      )
f_iface_ = __format_with_translation_function( iface_ )
f_tip_   = __format_with_translation_function( tip_   )
f_data_  = __format_with_translation_function( data_  )
