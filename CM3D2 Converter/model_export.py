...
class CNV_OT_export_cm3d2_model(bpy.types.Operator):
    ...
    def draw(self, context):
        ...
        box.prop(self , 'is_align_to_base_bone', icon='OBJECT_ORIGIN')
        box.prop(self , 'is_convert_tris'      , icon='MESH_DATA')
        box.prop(self , 'is_split_sharp'       , icon='MOD_EDGESPLIT')
        box.prop(self , 'export_tangent'       , icon='CURVE_BEZCIRCLE')
        sub_box = box.box()
        sub_box.prop(self , 'shapekey_threshold'     , icon='SHAPEKEY_DATA', slider=True)
        sub_box.prop(prefs, 'skip_shapekey'          , icon='SHAPEKEY_DATA', toggle=1)
        sub_box.prop(self , 'export_shapekey_normals', icon='NORMALS_VERTEX_FACE')
        row = sub_box.row()
        row    .prop(self , 'shapekey_normals_blend' , icon='MOD_NORMALEDIT', slider=True)
        row.enabled = self.export_shapekey_normals
        row = sub_box.row()
        row    .prop(self , 'use_shapekey_colors'    , icon='GROUP_VCOL', toggle=0)
        row.enabled = self.export_shapekey_normals
        sub_box = box.box()
